...
## Open questions
- Where exactly the Piola-type transform (or NeoPZ's variant) is applied: element `ComputeRequiredData`? shape classes? → decisive for [[piola-transformations]] (Phase 4).
- Sign/orientation convention for shared faces (unit tests suggest permutation-invariance is enforced — how?).
