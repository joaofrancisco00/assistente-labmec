...
- **M8 `Material/needrefactor/` legacy island**: 19+108 files [✓ count], duplicate physics (two `TPZMixedDarcyFlow`s), old naming, the reentrancy statics — self-labeled debt that still compiles into `pz` and shadows modern classes by include-path accident risk.
